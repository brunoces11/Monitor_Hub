...
read_containers() {
  docker ps -a --no-trunc --format '{{.ID}}\t{{.Names}}\t{{.State}}\t{{.RunningFor}}\t{{.Status}}'
}

normalize_state() {
  local state="${1:-}"
  case "$state" in
    running|paused|exited|restarting|dead|created)
      printf '%s' "$state"
      ;;
    *)
...
mem_bytes_from_human() {
  local raw="${1:-0}"
  raw="${raw//,/}"
  if [[ "$raw" == "0B" || "$raw" == "0" || -z "$raw" ]]; then
    printf '%s' 0
    return
  fi

  awk -v value="$raw" '
    function pow(base, expo,   i, result) {
      result = 1
      for (i = 0; i < expo; i++) result *= base
      return result
    }
    {
      if ($0 ~ /GiB$/) { sub(/GiB$/, "", value); print value * pow(1024, 3); exit }
      if ($0 ~ /MiB$/) { sub(/MiB$/, "", value); print value * pow(1024, 2); exit }
      if ($0 ~ /KiB$/) { sub(/KiB$/, "", value); print value * 1024; exit }
      if ($0 ~ /B$/) { sub(/B$/, "", value); print value + 0; exit }
      print value + 0
    }
  '
}

{
  ...
  while IFS=$'\t' read -r container_id container_name container_state running_for container_status; do
    ...
    container_state="$(normalize_state "$container_state")"
    ...
    if [[ "$container_state" != "running" && "$container_state" != "paused" ]]; then
      cpu_perc="0.00%"
      mem_usage="0B / 0B"
      mem_perc="0.00%"
    elif [[ "$stats_line" == "$cpu_perc" ]]; then
      cpu_perc="0.00%"
      mem_usage="0B / 0B"
      mem_perc="0.00%"
    fi
    ...
    mem_bytes="$(mem_bytes_from_human "$mem_current")"
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
      "$(priority_for_state "$container_state")" \
      "$mem_bytes" \
      ...
      "$container_id" >> "$records_file"
  done < <(read_containers)

  while IFS=$'\t' read -r _priority _mem_bytes container_name container_state cpu_perc mem_current mem_perc uptime_text container_id; do
    ...
    printf '      "uptime": "%s"\n' "$(json_escape "$uptime_text")"
    printf '    }'
  done < <(sort -t $'\t' -k1,1n -k2,2nr -k3,3 "$records_file")
